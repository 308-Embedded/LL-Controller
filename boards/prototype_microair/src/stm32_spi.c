...
void stm32_spidev_initialize(void)
{
  ...
  stm32_configgpio(BMI088_ACS);
  stm32_configgpio(BMI088_GCS);
#endif
#ifdef CONFIG_STM32H7_SPI3
  spiinfo("Configure GPIO for SPI3/CS\n");
  stm32_configgpio(BMI270_CS);
                      ...
                      uint32_t devid, bool selected)
{
  switch (devid)
    {
      case 0 : // acc
        spiinfo("bmi088 acc %s\n",
                selected ? "on" : "off");

        /* Set the GPIO low to select and high to de-select */
        stm32_gpiowrite(BMI088_ACS, !selected);
        break;
      case 1 : // gyro
        spiinfo("bmi088 gyro %s\n",
                selected ? "on" : "off");
        stm32_gpiowrite(BMI088_GCS, !selected);
        break;
      default:
        break;
    } 
  ...
  switch (devid)
    {
      case 0:
        ...
        /* Set the GPIO low to select and high to de-select */
        stm32_gpiowrite(BMI270_CS, !selected);
        break;
